...
def GenerateVdwNeighborList( mol, atomi, scalefac, moltype = 'OEMol'):
    ...
    if moltype is 'OEMol':
        ...
        scaledRadi = atomi.GetProp('radius')*scalefac
        # get xyz for atom i
...
def CullClashPts( mol, nbrList, scalefac, pts, moltype = 'OEMol'):
    ...
    if moltype is 'OEMol':
        ...
        for atom in nbrList:
            ...
            nbrXyz.append( list(xyztmp) )
        # precalculate neighbor scaled squared radii
        nbrRadiusSq = [ atom.GetProp('radius')*scalefac*atom.GetProp('radius')*scalefac for atom in nbrList]
    ...
    while shellScale<(options['outer']+.000001):
        ...
        for atom in mol.GetAtoms():
            ...
            if moltype is 'OEMol':
                ...
                xyz = pos[atom.GetIdx()]
                scaledRad =  atom.GetProp('radius')*shellScale
...
def GenerateCutoffs( mol, options, moltype = 'OEMol'):
    ...
    for atom in mol.GetAtoms():
        ...
        if moltype is 'OEMol':
            ...
            atom.SetProp('PointSetInnerRad', inner)
            atom.SetProp('PointSetOuterRad', outer)
    return mol
