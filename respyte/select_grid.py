...
def SelectGridPts(mol, inner, outer, pts, radiiType): # Can use Alvarez's Radii here!!!
    """ select pts inside the region definded by inner and outer radius of all atoms in xyzs.
    For now, let's say mol is an object generated from molecule_resp.py. (modified CIB's CullGridPts) """
    culled = []
    xyzs = mol.xyzs[0]
    innersSq  = []
    outersSq = []
    for elem in mol.elems[0]:
