...
##########################################################################################################
    ...
    def write_mol2(self, mol, qpot, moltype='oemol', outfile= 'out.mol2'):
        if moltype is 'oemol':
            # set partial charges
            ...
            # Or use fbmol
            out = []
            row_format = "%7d %-5s    %9.4f %9.4f %9.4f %-7s %2d %4s  %12.4f"
            out.append('@<TRIPOS>MOLECULE')
            out.append('Mol2 generated by resp_optimizer')
            out.append('%i %i 0 0 0' % (len(mol.elem), len(mol.bonds)))
            out.append('SMALL')
            out.append('RESP_CHARGES\n')
            out.append('@<TRIPOS>ATOM')
            for idx, i in enumerate(mol.xyzs[0]):
                out.append(row_format % (idx+1, mol.elem[idx], i[0], i[1], i[2], mol.elem[idx], mol.resid[idx], mol.resname[idx], round(qpot[idx],4))) 
            out.append('@<TRIPOS>BOND')
            bond_format = "%6d %5d %5d %4d"
            for idx, i in enumerate(mol.bonds):
                out.append(bond_format % (idx+1, i[0]+1, i[1]+1, 1))
            ...
            for xyz,gridxyz, espval, efval  in zip(self.molecule.xyzs[loc: loc+i], self.molecule.gridxyzs[loc: loc+i], self.molecule.espvals[loc: loc+i], self.molecule.efvals[loc: loc+i]):
                ...
                if writeMol2 is True:
                    if self.inp.cheminformatics == 'openeye':
                        ...
                        molt = 'fbmol' 
                    self.write_mol2(self.molecule.mols[idx], qpots[idx], moltype = molt,
            ...
            for xyz,gridxyz, espval, efval  in zip(self.molecule.xyzs[loc: loc+i], self.molecule.gridxyzs[loc: loc+i], self.molecule.espvals[loc: loc+i], self.molecule.efvals[loc: loc+i]):
                ...
                if writeMol2 is True:
                    if self.inp.cheminformatics == 'openeye':
                        ...
                        molt = 'fbmol'    
                    self.write_mol2(self.molecule.mols[idx], qpots[idx], moltype = molt,                                                                                            outfile = '%s/resp_output/mol%d_conf%d.mol2'% (path, idx+1, config))
                config += 1
            loc += i
        return 0
