...
def xyztopdb(filename):
    xyzmol = ReadMolFromFile(filename)
    xyzmol.resname = ['MOL' for i in range(len(xyzmol.elem))]
    xyzmol.resid = [1 for i in range(len(xyzmol.elem))]
    name = os.path.splitext(filename)[0]
    xyzmol.write('%s.pdb' % name)
    pdbmol = Molecule('%s.pdb' % name)
    return pdbmol
