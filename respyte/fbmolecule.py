...
AtomVariableNames = {'elem', 'partial_charge', 'atomname', 'atomtype', 'tinkersuf', 'resid', 'resname', 'qcsuf',
                     'qm_ghost', 'chain', 'altloc', 'icode', 'terminal', 'vdwradii'}
#=========================================#
...
def format_mol2_atom(atom_id, atom_name, xyz, atom_type, subst_id, subst_name, charge): # HJ
    return "%7d %-5s    %9.4f %9.4f %9.4f %-7s %2d %4s  %12.4f" % (atom_id, atom_name, xyz[0], xyz[1], xyz[2], atom_type, subst_id, subst_name, charge)
def format_mol2_bond(bond_id, origin_atom_id, target_atom_id, bond_type): # HJ
    return "%6d %5d %5d %4d" %(bond_id, origin_atom_id, target_atom_id, bond_type)
...
class Molecule(object):
    ...
    def add_vdw_radii(self, radiiType='bondi'):
        vdwradii = []
        if 'elem' in self.Data:
            AtomicNum = [Elements.index(i) for i in self.elem]
            for i, an in enumerate(AtomicNum):
                if radiiType == 'bondi':
                    vdwradii.append(BondiRadii[an-1])
                elif radiiType == 'Alvarez':
                    vdwradii.append(AlvarezRadii[an-1])
                else:
                    raise NotImplementedError('%s is not implemented yet;A; Why dont you use bondi or Alvarez?' % (radiiType))

        self.vdwradii = vdwradii
        #self.Data['vdwradii'] = vdwradii
    ...
    def write_mol2(self, selection, **kwargs): # HJ

        # When converting from mol2 to xyz in interactive prompt,
        # ask user for some MOL2-specific info.
        if sys.stdin.isatty():
            self.require('xyzs')
            self.require_resname()
            self.require_resid()
        ...
        # Atomtype
        if 'atomtype' not in self.Data:
            self.atomtype = self.elem
        # Partial charges
        if 'partial_charge' not in self.Data:
            charge_type = 'NO_CHARGE'
            qpot = list(0.0 for i in self.elem)
        else:
            charge_type = 'USER_CHARGE'
            qpot = self.partial_charge
        out = []
        for I in selection:
            out.append('@<TRIPOS>MOLECULE')
            out.append('Mol2 generated by resp_optimizer')
            out.append('%i %i 0 0 0' % (len(self.elem), len(self.bonds)))
            out.append('SMALL')
            out.append('%s\n' % charge_type)
            out.append('@<TRIPOS>ATOM')
            for idx, xyz in enumerate(self.xyzs[I]):
                out.append(format_mol2_atom(idx+1, self.elem[idx], xyz, self.atomtype[idx], self.resid[idx], self.resname[idx], round(qpot[idx],4)))
            out.append('@<TRIPOS>BOND')
            for idx, i in enumerate(self.bonds):
