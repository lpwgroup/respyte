...
def GenerateVdwNeighborList( mol, atomi, scalefac):
    ...
    Input arguments:
      mol: the OEMol whose atoms are examined to see if they are neighbors.
...
def GenerateMSKShellPts( mol, options):
    ...
    Input arguments:
      mol: the OEMol molecule for which we want the MSK point set.
    ...
    while shellScale<(options['outer']+.000001):
        ...
        for atom in mol.GetAtoms():
            ...
            nbrs = GenerateVdwNeighborList(mol, atom, shellScale)
            atomPts = GenerateConnollySphere( scaledRad, xyz, density)
            culledPts = CullClashPts( mol, nbrs, shellScale, atomPts)
...
def GenerateCutoffs( mol, options):
    '''Sets inner and outer boundary radii on each OEAtom as prescribed by the options.
    Input arguments:
      mol: the OEMol molecule for which the atomic boundary radii will be set.
...
def GenerateBoxMinMax( mol):
    ...
    Input arguments:
      mol: the OEMol molecule for which the box dimensions are desired.
...
def CullGridPts( mol, pts):
    ...
    Input arguments:
      mol: the OEMol with inner and outer boundaries set on each atom.
      pts: the list of points to cull
...
def GenerateGridPointSetAroundOEMol(mol, options):
    ...
    molPrep = GenerateCutoffs( mol, options)
    # based on the cutoffs, get the min and max box dimensions for the grid
    boxmin, boxmax = GenerateBoxMinMax(molPrep)
    ...
    cullPts = CullGridPts( molPrep, gridPts)
    return cullPts
